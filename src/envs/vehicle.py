...
class VehliceEnv(gym.Env):
    ...
    def __init__(
        ...
        self._AG = np.eye(self.nx) + Ac * self.dt
        self._BG = Bc * self.dt
        self._CG = None
    ...
    def step(self, u):
        e, edot, etheta, ethetadot = self._state

        u = np.clip(u, -self.max_steering, self.max_steering)[0]
        costs = 0.01 * e**2 + 1/25.0 * edot**2 + etheta**2 + 1/25.0 * ethetadot**2 + 2.0/(np.pi/6.0)**2 * (u*self._factor)**2 - 5.0#2.0/(np.pi/6.0)**2 * (u*self.factor)**2 - 5.0
    ...
    def reset(self, seed=None):
        ...
        high = np.array([1.0, 0.5, 0.1, 0.5]) * self._factor
...
class VehliceEnvPartial(VehliceEnv):
    def __init__(self, factor=1):
        super().__init__(factor=factor)
        # outputs are e and etheta
        #self.CG = np.array([[1, 0, 0, 0], [0, 0, 0, 1]])
        self._CG = np.array([[1, 0, 0, 0], [0, 0, 1, 0]])
...
class VehliceEnvPartialNorm(VehliceEnvPartial):
    def __init__(self, factor=1):
        super().__init__(factor=factor)

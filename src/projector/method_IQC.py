...
class ProjectorIQC(object):
    ...
    def __init__(self, xi_dim, z_dim, env, Q10=None, Q20=None, Aphi=0, Bphi=1, loss=None):
        ...
        self.Ce1 = env._Ce1
        self.De1 = env._De1
        self.Ce2 = env._CG2 if env._CG2 is not None else np.block([[np.eye(self.xe_dim), np.zeros(self.xe_dim, self.psi_dim)]])
        self.M   = env._M
    ...
    def updateRNN(self, rnn, clip_value=0.2):
        ...
        return rnn.set_weights(
            torch.FloatTensor(AK), torch.FloatTensor(BK1),
            torch.FloatTensor(BK2),
            torch.FloatTensor(CK1), torch.FloatTensor(DK1),
            torch.FloatTensor(DK2),
